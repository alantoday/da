...
if(isset($_GET['product']) && $_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if (empty($error) && $_POST['payment_type']=="check_wire") {
		...
		if (empty($_POST['inf_invoice_id'])) {
			...
			if (!$sub_plan) {
				...
				if ($subscriptions_array = InfGetRecurringOrders($mrow['inf_contact_id'], _WriteProductId($db, $_GET['product']))) {
					...
					$days_till_charge = 0;
					$new_inf_sub_id = InfCreateRecurringOrder($mrow['inf_contact_id'], $_POST['inf_sub_plan_id'], $sub_plan_prices[$_POST['inf_sub_plan_id']], $_POST['card_id'], $days_till_charge);
					$inf_sub_id = $new_inf_sub_id;
		...
		if (empty($error) && $_POST['cvv']<>"xxx") {
			...
			InfUpdateCreditCardCVV ($_POST['card_id'], $_POST['cvv']);
		}
		if (empty($error)) {			
			if ($sub_plan) {
			...
			if (!empty($_POST['deposit_amt'])) {
				$charge_res = Infusionsoft_InvoiceService::chargeInvoiceArbitraryAmount($mrow['inf_contact_id'], $_POST['inf_invoice_id'], $_POST['card_id'], $_POST['deposit_amt'], INF_MERCHANT_ID, "Deposit");
			} else {
...
if ($new_inf_sub_id && !empty($error)) {
	# Delete Subscription plan if one is created but there is an error
	Infusionsoft_InvoiceService::deleteSubscription($new_inf_sub_id);						
...
<?php if (!empty($_GET['deposit'])) { ?>    
    <tr id='cards_select_2' <?php if ($check_wire_only) echo "style='display:none'"; ?>>
      <td width='135px'>Amount:</td>
      <td><select name="deposit_amt">
          <?php echo WriteSelect(isset($_POST['deposit_amt']) ? $_POST['deposit_amt'] : "", array(array(""=>"Full Amount"), array(1000=>"$1,000 Deposit")), false, false); ?>
        </select>
    </tr>
<?php } ?>    
    <tr id='cards_select_2' <?php if ($check_wire_only) echo "style='display:none'"; ?>>
