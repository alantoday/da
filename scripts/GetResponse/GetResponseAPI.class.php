...
{	
	...
	*/
	 ...
	 * @return object 
	 */
	public function createCampaign($operator = 'CONTAINS', $comparison = '%')
	{
		$params = null;
		if(in_array($operator, $this->textOperators)) $params = array('name' => array($operator => $comparison));
		$request  = $this->prepRequest('add_campaign', $params);
		$response = $this->execute($request);
		return $response;
	}

	# New
	public function addCampaign($name, $description, $from_field, $reply_to_field, $confirmation_subject, $confirmation_body, $language_code) {
        $params = array(
            'name' => $name,
	...
	public function addAutoresponder($campaign, $subject, $cycle_day, $html = null, $plain = null, $from_field = null, $name, $flags = null)
	{
		...
		if(is_string($name)) $params['name'] = $name;
	...
	public function addContact($campaign, $name = '', $email, $action = 'standard', $cycle_day = 0, $customs = array())
	{
		...
		# DA ADDED to stop double-opt in on adding contact via API
		$params['consumer_key'] = GETRESPONSE_CONSUMER_KEY;
	...
	*/
    ...
    public function get_confirmation_subject($id) {
        $request = $this->prepRequest('get_confirmation_subject', array('confirmation_subject' => $id));
        $response = $this->execute($request);
        return $response;
    }

    public function get_confirmation_subjects() {
        $request = $this->prepRequest('get_confirmation_subjects', array('language_code' => array('EQUALS' => 'EN')));
        $response = $this->execute($request);
        return $response;
    }

    public function get_confirmation_body($id) {
        $request = $this->prepRequest('get_confirmation_body', array('confirmation_body' => $id));
        $response = $this->execute($request);
        return $response;
    }

    public function get_confirmation_bodies() {
        $request = $this->prepRequest('get_confirmation_bodies', array('language_code' => array('EQUALS' => 'EN')));
        $response = $this->execute($request);
        return $response;
    }
